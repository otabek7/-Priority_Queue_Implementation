...
private:
    ...
    {
        // TODO
        if (index != 0 && comp(c[parent(index)], c[index]))
        {
            T temp = std::move(c[parent(index)]);
    ...
    {
        // TODO
        if (left_child(index) >= c.size() && right_child(index) >= c.size())
            return;
        // left and right
        else if (right_child(index) >= c.size() && left_child(index) < c.size())
        {
            if (comp(c[index], c[left_child(index)]))//compare both
            {
                T temp = std::move(c[left_child(index)]);
                c[left_child(index)] = std::move(c[index]);
                c[index] = std::move(temp);
            }
        }
        // Left and right not
        ...
        {
            if (comp(c[left_child(index)], c[right_child(index)]))//compare both
            {
                if (comp(c[index], c[right_child(index)]))//compare again
                {
                    T temp = std::move(c[right_child(index)]);
                    c[right_child(index)] = std::move(c[index]);
                    c[index] = std::move(temp);
            ...
            {
                if (comp(c[index], c[left_child(index)]))//compare again
                {
                    T temp = std::move(c[left_child(index)]);
                    c[left_child(index)] = std::move(c[index]);
                    c[index] = std::move(temp);
